...
class BackendService {
  ...
  async initializeDatabase(): Promise<boolean> {
    ...
    if (!technovaUser) {
      ...
      this.setStorage(STORAGE_KEYS.USERS, users);
    } else if (technovaUser.id !== 'c_technova') {
    ...
    // Force update exercises if stale
    const currentExercises = this.getStorage<PracticeExercise[]>(STORAGE_KEYS.EXERCISES, []);
    const isExerciseStale = currentExercises.some(ex => ex.id === 'momo-2' && ex.assumption === 'Công ty MoMo E-Wallet');
    if (isExerciseStale) {
      this.getExercises(); // This will trigger the update logic in getExercises()
    }
    ...
    const extraExercises: PracticeExercise[] = [
      {
        id: 'momo-2',
        company: 'Mindtrace',
    ...
    const allExercises = [momoExercise, ...extraExercises, ...exercises];
  ...
  register(userData: { email: string; name: string; role: 'student' | 'business'; password?: string }): User {
    ...
    const newUser: any = {
      ...
      subscriptionTier: userData.role === 'business' ? 'FREE' : undefined,
  ...
  getExercises(): PracticeExercise[] {
    ...
    // Check if any momo exercise is missing assumption or has old tag
    const isStale = exercises.some(ex => 
      ex.id.startsWith('momo-') && (!ex.assumption || (ex.id === 'momo-6' && ex.tag === 'Fullstack Dev'))
    );
    ...
    if (!hasMomo || exercises.length < 6 || isStale) {
      ...
      // Filter out existing momo exercises to avoid duplicates if we're just updating
  ...
  canPostJob(userId: string): boolean {
    ...
    // If no last reset date, or if month/year has changed, reset the limit
    if (!lastReset || now.getMonth() !== lastReset.getMonth() || now.getFullYear() !== lastReset.getFullYear()) {
      this.resetMonthlyLimits(userId);
      // After reset, we need to get the updated user object
      const updatedUser = this.getUserById(userId);
      return (updatedUser?.postsRemaining || 0) > 0;
    }
