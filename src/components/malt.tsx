...
const Malt: React.FC<MaltProps> = ({ 
  ...
  const isUserPremium = currentUser.subscriptionTier && currentUser.subscriptionTier !== 'FREE';
      ...
      <aside className="w-64 border-r border-slate-800 bg-[#111821] flex flex-col h-full shrink-0">
        ...
        <nav className="flex-1 px-4 space-y-1">
          ...
          <button 
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[#1392ec] text-white font-semibold text-left shadow-lg shadow-[#1392ec]/20"
          >
        ...
        {!isUserPremium && (
          <div className="m-4 p-4 bg-gradient-to-br from-emerald-500/20 to-blue-500/20 border border-emerald-500/30 rounded-2xl">
            <div className="flex items-center gap-2 mb-2">
              <span className="material-symbols-outlined text-primary text-lg">verified</span>
              <span className="text-[10px] font-black text-white uppercase tracking-widest">Premium Plan</span>
            </div>
            <p className="text-[10px] text-slate-400 leading-relaxed mb-3">Mở khóa toàn bộ kho đề bài thực tế từ các doanh nghiệp hàng đầu.</p>
            <button 
              onClick={() => onSelectExercise('upgrade')}
              className="w-full py-2 bg-primary text-background-dark rounded-lg text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all"
            >
              Nâng cấp ngay
            </button>
          </div>
                  ...
                  <div key={item.id} className="bg-[#111821] border border-slate-800 rounded-2xl overflow-hidden hover:border-[#1392ec]/50 transition-all flex flex-col group shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div className="h-40 bg-slate-800 relative overflow-hidden">
                      ...
                      {item.isPremium && !isUserPremium && (
                        <div className="absolute top-4 right-4">
                          <div className="bg-amber-500/20 text-amber-400 px-2 py-1 rounded-lg flex items-center gap-1 border border-amber-500/30 backdrop-blur-md">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      <div className="flex flex-col gap-1.5 mb-4">
                        ...
                        {item.assumption && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black text-[#1392ec] uppercase tracking-tight italic">{item.assumption}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-auto flex items-center justify-between pt-6 border-t border-slate-800/50">
                        ...
                        >
                          {item.isPremium && !isUserPremium ? <span className="material-symbols-outlined text-sm">lock</span> : null}
                          {status ? 'Làm lại' : (item.isPremium && !isUserPremium ? 'Mở khóa' : 'Thử sức')}
                          <span className="material-symbols-outlined text-base">
                            {status ? 'refresh' : (item.isPremium && !isUserPremium ? 'verified' : 'play_arrow')}
                          </span>
                        </button>
                      </div>
                    </div>
