...
interface RegisterPageProps {
  ...
  onRegisterSuccess: (role: 'student' | 'business', email: string) => void;
}

const RegisterPage: React.FC<RegisterPageProps> = ({ onBack, onNavigateToLogin, onRegisterSuccess }) => {
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden bg-background-dark text-slate-100">
        ...
        <div className="w-full max-w-[480px] relative z-10 flex flex-col items-center text-center">
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-6 relative overflow-hidden bg-background-dark text-slate-100">
      ...
      <div className="w-full max-w-[480px] relative z-10 flex flex-col items-center">
        ...
        <div className="login-card w-full p-8 md:p-10 rounded-[2.5rem] border border-white/10 shadow-2xl">
          <h1 className="text-2xl md:text-3xl font-bold text-center mb-10 text-white tracking-tight">Đăng ký tài khoản MindTrace</h1>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <button 
              type="submit"
              className={`w-full py-4 text-white font-bold rounded-2xl hover:opacity-90 transition-all shadow-xl mt-4 ${
          ...
          <div className="relative mt-8 mb-8 text-center">
            ...
            <span className="relative px-4 text-xs font-bold text-slate-500 bg-[#121212] uppercase tracking-widest">Hoặc đăng ký với</span>
          </div>

          <button 
            type="button"
            onClick={() => onRegisterSuccess(role, "google-auth-user@gmail.com")}
        ...
        <p className="mt-8 text-slate-400 font-medium">
          Đã có tài khoản?
          <button onClick={onNavigateToLogin} className="text-primary font-bold hover:underline ml-1">Đăng nhập ngay</button>
        </p>
      </div>
    </div>
  );
