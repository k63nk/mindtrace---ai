  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isJobTest && application && job) {
        ...
        // AI Grading only for practice exercises
        const result = await evaluatePracticeSolution(
          targetTitle,
          `Nội dung bài làm từ file: ${fileName}. Đây là bài làm cho bài tập ${targetTitle}.`
        );
          ...
          <div className="bg-[#111821] border border-slate-800 rounded-[2.5rem] w-full max-w-3xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl">
            <div className="p-10">
              ...
              <div className="mb-8 p-6 bg-slate-800/50 rounded-2xl border border-slate-700">
                <p className="text-sm text-slate-300 leading-relaxed italic">"{evaluationResult.feedback}"</p>
              </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {exercise?.assumption && (
                  <>
                    <span className="w-1 h-1 bg-slate-700 rounded-full"></span>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Giả định: {exercise.assumption}</span>
                  </>
                )}
              </div>
            </div>
