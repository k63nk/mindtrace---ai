                            ...
                            <div className="flex flex-col gap-2">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {app.testScore !== undefined && (
                                  <span className="inline-flex items-center px-2 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[9px] font-black text-blue-400 uppercase tracking-widest">
                                    Test: {app.testScore}
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="p-8 bg-slate-900/50 border border-slate-800 rounded-2xl space-y-8">
                      {selectedApp.status === 'HIRED' || selectedApp.status === 'FAILED' ? (
                        <div className="space-y-8 animate-in fade-in duration-500">
                          <div className="p-6 bg-[#0f172a] rounded-2xl border border-slate-800 flex items-center justify-between">
                            <div>
                              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Điểm bài test</p>
                              <p className="text-3xl font-black text-[#1392ec] italic">{selectedApp.testScore}/100</p>
                            </div>
                            <div className="text-right">
                              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Quyết định</p>
                              <span className={`inline-flex items-center px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border ${
                                selectedApp.status === 'HIRED' 
                                ? 'bg-emerald-500/20 border-emerald-500 text-emerald-500' 
                                : 'bg-red-500/20 border-red-500 text-red-500'
                              }`}>
                                {selectedApp.status === 'HIRED' ? 'Trúng tuyển' : 'Trượt'}
                              </span>
                            </div>
                          </div>
                          
                          {selectedApp.companyFeedback && (
                            <div className="p-6 bg-slate-800/30 rounded-2xl border border-slate-800">
                              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Nhận xét từ doanh nghiệp</p>
                              <p className="text-sm text-slate-300 leading-relaxed italic">"{selectedApp.companyFeedback}"</p>
                            </div>
                          )}
                          ...
                          <div className="pt-4">
                            <button 
                              ...
                              className="w-full py-4 bg-slate-800 text-slate-400 rounded-xl font-black uppercase text-xs tracking-[0.2em] hover:bg-slate-700 transition-all border border-slate-700"
                            >
                              Đóng chi tiết
                            </button>
                          </div>
